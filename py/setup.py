...
"""Build uniarchive._core, a Cython extension over the UniArchive C ABI.
...
PKG_DIR = os.path.join(HERE, "uniarchive")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniArchive.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniArchive.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniArchive.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniArchive.so", True
...
def nim_project_dir():
    """Where UniArchive.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building uniarchive from "
...
# The sdist ships the pre-transpiled uniarchive/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("uniarchive", "_core.pyx")
ext = Extension(
    "uniarchive._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("uniarchive", "_core.c")],
    include_dirs=[INCLUDE],
    library_dirs=[LIB_DIR],
    extra_link_args=LINK_ARGS,
    libraries=["UniArchive"],
)
...
setup(
    ...
    package_data={"uniarchive": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"uniarchive": ["_core.c"]},
    zip_safe=False,
)
